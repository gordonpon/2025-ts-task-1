...
console.log('cateKeyName', catKeyName); // LargePlant
...
export const sumbitProduct = (
  ...
  product: CreateProduct | UpdateProduct
    ...
    if (type === "create") {
      const CreateProduct = product as CreateProduct;
      return `新增商品成功：${CreateProduct.title}`;
    } else {
      const UpdateProduct = product as UpdateProduct;
      return `更新商品成功：${UpdateProduct.id}`;
    }
  }
